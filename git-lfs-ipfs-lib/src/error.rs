use actix_web::{client::SendRequestError, error::JsonPayloadError, error::PayloadError};
use failure::Fail;
...
pub enum Error {
    ...
    #[fail(display = "error in receiving a response from the IPFS API {:?}", _0)]
    ...
    #[fail(display = "error while sending a request to the IPFS API {:?}", _0)]
